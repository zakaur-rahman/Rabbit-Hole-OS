...
const typeIcons: Record<string, React.ReactNode> = {
    article: <FileText size={14} className="text-(--green)" />,
    video: <Video size={14} className="text-(--red)" />,
    code: <Code size={14} className="text-(--amber)" />,
    product: <ShoppingBag size={14} className="text-(--blue)" />,
    academic: <BookOpen size={14} className="text-(--blue)" />,
    note: <StickyNote size={14} className="text-(--amber)" />,
    ghost: <Ghost size={14} className="text-(--muted)" />,
};
        ...
        <div className="h-full flex flex-col bg-(--bg) text-(--text) overflow-hidden">
            ...
            <div className="p-3 border-b border-(--border) flex flex-col gap-3">
                <button
                    ...
                    className="w-full h-9 flex items-center justify-center gap-2 bg-[rgba(232,160,32,0.05)] hover:bg-[rgba(232,160,32,0.1)] text-(--amber) rounded-[6px] text-[13px] font-bold transition-all border border-[rgba(232,160,32,0.2)]"
                >
                    <StickyNote size={14} className="text-(--amber)" fill="rgba(232,160,32,0.2)" />
                    Open Today&apos;s Note
                </button>
                <div className="relative">
                    <Search size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-(--sub)" strokeWidth={1.5} />
                    <input
                        type="text"
                        placeholder="Filter files..."
                        value={search}
                        onChange={(e) => setSearch(e.target.value)}
                        className="w-full h-[28px] bg-[#1a1918] border border-(--border) rounded-[4px] pl-8 pr-3 text-[12px] font-mono text-(--text) focus:outline-none focus:border-(--green) transition-colors placeholder:text-(--sub) placeholder:tracking-wide font-medium"
                    />
                </div>
            </div>
                    ...
                    return (
                        <div key={type} className="flex flex-col">
                            <button
                                ...
                                className="w-full flex items-center h-7 px-1 rounded hover:bg-(--raised) transition-colors group"
                            >
                                <div className="w-4 flex justify-center text-(--sub) transition-transform mr-1">
                                ...
                                <span className="text-[11px] font-bold text-[#b5a999] group-hover:text-(--text) uppercase tracking-[0.15em] flex-1 text-left">
                                    {type}S
                                </span>
                                <span className="text-[10px] text-(--sub) font-mono bg-(--raised) border border-(--border2) px-1.5 rounded-[4px] ml-2">
                                    {typeNodes.length}
                                </span>
                            </button>
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`w-full h-[26px] flex items-center px-2 transition-colors text-left group/item relative ${
                                                    isActive 
                                                        ? 'bg-[#2a2723] border-l-2 border-(--amber)' 
                                                        : 'hover:bg-(--raised) border-l-2 border-transparent'
                                                }`}
                                            >
                                                <div className="w-4 mr-0.5 opacity-0 pointer-events-none" /> {/* Indent */}
                                                {(node.type && typeIcons[node.type]) || <FileText size={14} className="text-(--sub)" />}
                                                <span className={`text-[12px] ml-2 truncate font-medium flex-1 ${isActive ? 'text-(--amber)' : 'text-[#888] group-hover/item:text-(--text)'}`}>
